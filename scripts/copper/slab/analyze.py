...
VACUUM_HEIGHT_PATH = PosixPath("data/copper/slab/vacuum_height")
FREE_LAYER_PATH = PosixPath("data/copper/slab/free_layer")
WIDTH_PATH = PosixPath("data/copper/slab/width")
SAVE_DIR = Path(__file__).parent / "figures"


def _analyze_convergence_with_vacuum_height(data_path: PosixPath) -> None:
    maybe_copy_files_from_hpc(data_path, data_path)
    calculators = load_all_calculators(data_path)

    fig, _, _ = plot_energy_against_vacuum_height(calculators)
    fig.tight_layout()
...
def _copy_slab_width_from_hpc(data_path: PosixPath) -> None:
    maybe_copy_files_from_hpc(data_path, data_path)
...
if __name__ == "__main__":
    ...
    _copy_slab_width_from_hpc(WIDTH_PATH)
